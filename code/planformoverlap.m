...
ZD_19 = ZD_19_2_dry;
clear('TDWB_19_2_chanMaps', 'ZD_19_2_dry');

%replace empty channel maps in treatment with channel map during timestep
%after
for i = 1:size(chanMaps_19,3); %I know 560 has a channel map, so I can start at 279 and replace with channel map that comes after, this will work for ones that have multiple no maps in a row
...
wetA_18 = [];
dryA_18 = [];
for i = 1:size(chanMaps_18_crop,3)
    wetA = nansum(nansum(chanMaps_18_crop(:,:,i)));
    dryA = A_18-wetA;
...
wetA_19 = [];
dryA_19 = [];
for i = 1:size(chanMaps_19_crop,3)
    wetA = nansum(nansum(chanMaps_19_crop(:,:,i)));
    dryA = A_19-wetA;
...
Phi18 = [];
corr18 = [];
for z=1:size(chanMaps_18_crop,3)
    baseline = chanMaps_18_crop(:,:,z);   
    for f=z:size(chanMaps_18_crop,3)
% STEP 1: Find the number of decorrelated pixels.
        transient = chanMaps_18(:,:,f);
...
Phi19 = [];
corr19 = [];
for z=1:size(chanMaps_19_crop,3)
    baseline = chanMaps_19_crop(:,:,z);   
    for f=z:size(chanMaps_19_crop,3)
% STEP 1: Find the number of decorrelated pixels.
        transient = chanMaps_19(:,:,f);
...
planform_overlap_18 = [hour18,changedpix18,Phi18,corr18];
planform_overlap_19 = [hour19,changedpix19,Phi19,corr19];

%Control Data
[ud,ix,iy] = unique(planform_overlap_18(:,1));  
%changed pix
mean_changedpix18 = [ud, accumarray(iy,planform_overlap_18(:,2),[],@mean)];
std_changedpix18 = [ud, accumarray(iy,planform_overlap_18(:,2),[],@std)];
%Phi
mean_Phi18 = [ud, accumarray(iy,planform_overlap_18(:,3),[],@mean)];
std_Phi18 = [ud, accumarray(iy,planform_overlap_18(:,3),[],@std)];
%Corr
mean_corr18 = [ud, accumarray(iy,planform_overlap_18(:,4),[],@mean)];
std_corr18 = [ud, accumarray(iy,planform_overlap_18(:,4),[],@std)];

%Treatment Data
[ud,ix,iy] = unique(planform_overlap_19(:,1));
%changed pix
mean_changedpix19 = [ud, accumarray(iy,planform_overlap_19(:,2),[],@nanmean)];
std_changedpix19 = [ud, accumarray(iy,planform_overlap_19(:,2),[],@nanstd)];
%Phi
mean_Phi19 = [ud, accumarray(iy,planform_overlap_19(:,3),[],@nanmean)];
std_Phi19 = [ud, accumarray(iy,planform_overlap_19(:,3),[],@nanstd)];
%Corr
mean_corr19 = [ud, accumarray(iy,planform_overlap_19(:,4),[],@nanmean)];
std_corr19 = [ud, accumarray(iy,planform_overlap_19(:,4),[],@nanstd)];
...
saveas(fig, 'planform_change.pdf')
